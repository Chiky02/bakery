...
type Tab = "listado" | "crear";

export function ClientesClient({ initial }: { initial: Cliente[] }) {
  const { panaderiaId } = useBakeryId();
  const { permisos, rol } = useBakery();
  const canList = hasPermiso("clientes", permisos, rol);
  const canCreate = hasPermiso("clientes_crear", permisos, rol);
  ...
  const [tab, setTab] = useState<Tab>(canList ? "listado" : "crear");
  ...
  async function guardar(e: React.FormEvent) {
    e.preventDefault();
    if (!canCreate || !panaderiaId || !form.nombre.trim()) return;
    const supabase = createClient();
    const payload = {
  ...
  const tabs: { id: Tab; label: string; show: boolean }[] = [
    ...
    { id: "crear", label: editId ? "Editar" : "Crear", show: canCreate },
  ];
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        {tab === "listado" && canCreate && (
          <Button type="button" onClick={goCrear}>
            Nuevo cliente
          </Button>
        )}
      </div>
